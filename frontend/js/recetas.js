...
const API = {
    ...
    INSUMOS_URL: 'http://localhost:3000/api/insumos',
...
function initRecetasEvents() {
    // Búsqueda de productos
    const searchInput = document.getElementById('productSearch');
    searchInput.addEventListener('input', function() {
        const searchTerm = this.value.trim();
        
        // Si el usuario está borrando texto, permitir que continúe
        if (searchTerm.length < selectedProductName.length) {
            selectedProductName = ''; // Limpiar el producto seleccionado
    ...
    // Evento para guardar cambios en el formulario de edición
    document.getElementById('editIngredientForm').addEventListener('submit', saveIngredientChanges);

    // Evento para agregar nuevo ingrediente
    document.getElementById('ingredientForm').addEventListener('submit', addIngredient);

    // Evento para eliminar desde el modal
    ...
    // Cargar productos e insumos al iniciar
    loadAllProductos();
    loadInsumos().then(() => {
